    ...
    modalPay.innerHTML = `
        <div style="background:#fff;padding:0;border-radius:30px;max-width:420px;width:100%;text-align:center;position:relative;box-shadow:0 20px 50px rgba(0,0,0,0.5);overflow:hidden;animation: slideUp 0.3s ease-out;box-sizing:border-box;">
            ...
            <div style="padding:20px; overflow-y:auto; max-height:75vh; box-sizing:border-box;">
                ...
                <div style="position:relative; margin-bottom:20px; display:inline-block; width:100%; height:80px; padding-top:20px;">
    
    <span class="emoji-stream" style="left:15%;  animation-delay: 0.0s;">💸</span>
    <span class="emoji-stream" style="left:30%;  animation-delay: 0.5s;">✨</span>
    <span class="emoji-stream" style="left:45%;  animation-delay: 1.0s;">💰</span>
    <span class="emoji-stream" style="left:60%;  animation-delay: 1.5s;">📱</span>
    <span class="emoji-stream" style="left:75%;  animation-delay: 2.0s;">💸</span>
    <span class="emoji-stream" style="left:25%;  animation-delay: 2.5s;">✨</span>
    <span class="emoji-stream" style="left:55%;  animation-delay: 3.0s;">💰</span>
    <span class="emoji-stream" style="left:70%;  animation-delay: 3.5s;">📱</span>

    <a href="${lienUssdAndroid}" class="btn-flash-mvola" style="display:block; width:100%; padding:18px; background:linear-gradient(135deg, #ffcc00 0%, #ffaa00 100%); color:#000; text-decoration:none; border-radius:16px; font-weight:800; font-size:1rem; box-shadow:0 8px 25px rgba(255,170,0,0.4); box-sizing:border-box; transition:all 0.3s cubic-bezier(0.175, 0.885, 0.32, 1.275); text-transform:uppercase; letter-spacing:0.8px; display:flex; align-items:center; justify-content:center; gap:10px; position:absolute; bottom:0; left:0; z-index:2;">
        <span style="font-size:1.2rem;">📞</span> 
        <span>Initier le paiement MVola</span>
...
function verifierEtAfficherAnnoncesSafeRun() {
    ...
    // --- INJECTION DIRECTE DES ANIMATIONS CSS DANS LA PAGE ---
    if (!document.getElementById('saferun-animation-styles')) {
        const styleSheet = document.createElement("style");
        styleSheet.id = "saferun-animation-styles";
        styleSheet.innerText = `
            /* Animation Coucou / Main qui fait signe */
            @keyframes saferunWavingHand {
                0%, 100% { transform: rotate(0deg); }
                20%, 60% { transform: rotate(14deg); }
                40%, 80% { transform: rotate(-8deg); }
            }
            /* Animation Zoom / Pulsation d'attention (Bouche / Promo) */
            @keyframes saferunPulse {
                0%, 100% { transform: scale(1); }
                50% { transform: scale(1.12); }
            }
            /* Animation Camion de livraison en mouvement */
            @keyframes saferunTruckDrive {
                0%, 100% { transform: translateX(0); }
                50% { transform: translateX(8px); }
            }
            /* Animation Portefeuille qui tremble (Urgence MVola) */
            @keyframes saferunWalletShake {
    ...
    if (!derniereCommande) {
        ...
        slidesData = [
            ...
            {
                icone: '<div class="saferun-anim-pulse"><i class="fas fa-clock" style="color:#ef4444;"></i></div>', background: '#fee2e2',
        ...
        slidesData = [
            {
                icone: '<div class="saferun-anim-pulse"><i class="fas fa-check-circle" style="color:#10b981;"></i></div>', background: '#e6f4ea',
                titre: "📄 Facture Payée & Validée",
                msg: messageSuivi
            },
            {
                icone: '<div class="saferun-anim-truck"><i class="fas fa-truck" style="color:#3b82f6;"></i></div>', background: '#dbeafe',
            ...
            {
                icone: '<div class="saferun-anim-pulse"><i class="fas fa-star" style="color:#eab308;"></i></div>', background: '#fef9c3',
...
(function() {
    ...
    window.addEventListener('mousemove', reinitialiserMinuteurInactivite);
    window.addEventListener('click', reinitialiserMinuteurInactivite);
    window.addEventListener('scroll', reinitialiserMinuteurInactivite);
    window.addEventListener('keypress', reinitialiserMinuteurInactivite);
    window.addEventListener('touchstart', reinitialiserMinuteurInactivite);
